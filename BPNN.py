...
class Neural_Net( ):
	def __init__( self, alpha=0.1, ip=5, h=5, op=5 ):
		self.alpha = alpha
		self.ipl = Layer( ip )
		self.hil = Layer( h )
	...
	def train( self, example ):
		...
		for ( index, node ) in enumerate( self.opl.nodes ):
			node.delta = ( example.op[ index ] - node.value ) * g_d( node.summation ) 
			for connection in node.i_connections:
		...
		for node in self.hil.nodes:
			...
			node.delta = g_d( node.summation ) * output_summation
			for connection in node.i_connections:
				connection.weight = connection.weight + self.alpha * connection.i_node.value * node.delta
